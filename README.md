...
## Сетки не появились в игре?

Самая частая причина — **Dota 2 была запущена во время обновления**. Игра
читает `hero_grid_config.json` только при старте и перезаписывает его старой
версией из памяти при выходе. Поэтому:

1. Полностью закройте Dota 2 и запустите утилиту ещё раз — обновлять сетку
   нужно при закрытой игре (утилита сама предупредит, если дота запущена).
2. Проверьте, что в игре выбран тот же аккаунт Steam, для которого обновлялась
   сетка (утилита обновляет все найденные аккаунты и пишет каждый путь в консоль).
3. Если для Dota 2 включён Steam Cloud — после обновления дождитесь
   синхронизации или перезапустите Steam.
4. Не помогло — пришлите файл `metagrid.log` (лежит рядом с exe) разработчикам:
   [github.com/paintdrip/dota2-metagrid-bot/issues](https://github.com/paintdrip/dota2-metagrid-bot/issues).
